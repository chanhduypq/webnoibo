...
abstract class CLogRoute extends CComponent
{
	...
	protected function formatLogMessage($message,$level,$category,$time)
	{
		if($level=='trace')
		{
			if(Yii::app()->getComponent('user',false)!==null)
			{
				$user=Yii::app()->getComponent('user',false);
				$user='Employee Number: '.$user->getName().' (ID: '.$user->getId().')'.' (IP: '. $_SERVER['REMOTE_ADDR'].')';
				return @date('Y/m/d H:i:s',$time)." [$level] [$category] $message $user \n";
			}
		}
		else 
		{

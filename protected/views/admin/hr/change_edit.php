            ...
            <div class="mainBox detail">
            	<div class="pageTtl">
                    ...
                    <span>
                        ...
                        ?>
                        <a class="btn btn-important" href="<?php echo Yii::app()->request->baseUrl; ?>/adminhr/change<?php echo $page;?>">
...
<script type="text/javascript"> 
        jQuery(function($){ 
            position1=getCookie("hr_change_edit_from_position");
            if(position1!=null&&position1!="null"){
                position1=position1;
            }
            else{
                position1='<?php echo $model->from_position;?>';
            }
            var str_id=$("#Change_from_unit").val();
            if(str_id!=""){
                $.ajax({    
				type: "POST", 
				async:true,
				...
				success: function(msg){	 
                                    ...
                                    for (i = 0,n=users.length; i <n; i++) {
                                        ...
                                        if(position1==users[i].id){
                                            jQuery('#Change_from_position').append("<option selected='selected' value=" + users[i].id + ">" + users[i].post_name + "</option>");
                                        }
                                        else{
            ...
            position2=getCookie("hr_change_edit_to_position");
            if(position2!=null&&position2!="null"){
                position2=position2;
            }
            else{
                position2='<?php echo $model->to_position;?>';
            }
            var str_id1=$("#Change_to_unit").val();
            if(str_id1!=""){
                $.ajax({    
				type: "POST", 
				async:true,
				url: "<?php echo Yii::app()->baseUrl;?>/adminuser/getposts/?unit_id="+str_id1,    				
				success: function(msg){	 
                                     users=$.parseJSON(msg);
                                    ...
                                    for (i = 0,n=users.length; i <n; i++) {
                                        ...
                                        if(position2==users[i].id){
                                            jQuery('#Change_to_position').append("<option selected='selected' value=" + users[i].id + ">" + users[i].post_name + "</option>");
                                        }
                                        else{
          ...
          $("#hr_form").attr('action','<?php echo Yii::app()->baseUrl;?>/adminhr/change_editconfirm/');       	   
		 
		      
           title=getCookie("hr_change_edit_member_name");
           if(title!=null&&title!="null"){
               $("#Change_member_name").val(title);
           }
           change_date=getCookie("hr_change_edit_change_date");
           ...
           detail=getCookie("hr_change_edit_detail");           
           if(detail!=null&&detail!="null"){
                detail1=detail.replace(/<br ?\/?>|_/g, '\n');
               $("#Change_detail").val(detail1);
           }
           division=getCookie("hr_change_edit_from_unit");
           if(division!=null&&division!="null"){
           $("#Change_from_unit").val(division);
           from_position=getCookie("hr_change_edit_from_position");
           $.ajax({    
				type: "POST", 
				async:true,
       ...
       division1=getCookie("hr_change_edit_to_unit");
           if(division1!=null&&division1!="null"){
           $("#Change_to_unit").val(division1);
           to_position=getCookie("hr_change_edit_to_position");
           $.ajax({    
				type: "POST", 
				async:true,
           ...
           setCookie("hr_change_edit_detail",detail); 
           ...
           $('button[type="submit"]').click(function(){  
			...
			$.ajax({    
				type: "POST", 
				async:true,
				url: "<?php echo Yii::app()->baseUrl;?>/adminhr/change_edit/?id=<?php echo $model->id;?>",    
				...
				success: function(msg){	           
					  ...
					  jQuery('#Change_detail').prev().remove();       
					  	...
					  	if(msg!='[]'){
                                                ...
                                                val=val.replace(/\n/g, "<br/>");
                                                setCookie("hr_change_edit_detail",val);

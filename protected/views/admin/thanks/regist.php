...
<?php
        ...
        $link_array[]=array("text"=>  Config::TEXT_FOR_ADD_IN_LINK_BACK_DIV,"link"=>'/adminthanks/regist') ;        
                ...
                <div class="cnt-box"> 
                    <div class="control-group">
                        ...
                        <div class="controls">
								...
								echo $form->dropDownList($model,'unit_id',$array_unit,  array('prompt'=>'Chọn chi nhánh - bộ phận')); 			
								?> 
                        </div>
                    </div>
                    <div class="control-group">
...
$("#Thanks_user_id").click(function() {
    ...
    if(options.length==1&&$("#Thanks_unit_id").val()==""){
        ...
        $("#Thanks_unit_id").change(function() {
           ...
           $("#thanks_form").attr('action','<?php echo Yii::app()->baseUrl;?>/adminthanks/registconfirm/');          
           sender=getCookie("thanks_regist_sender");
           if(sender!=null&&sender!="null"){
               $("#Thanks_sender").val(sender);
           }
           comment=getCookie("thanks_regist_comment");         
           if(comment!=null&&comment!="null"){
               comment1=comment.replace(/<br ?\/?>|_/g, '\n');         
               $("#Thanks_comment").val(comment1);
           }
           unit_id=getCookie("thanks_regist_unit_id");     
           if(unit_id!=null&&unit_id!="null"){
            ...
            unit_id=str_id[0];
            position_id=str_id[1];
               user_id=getCookie("thanks_regist_user_id"); 
               $.ajax({    
				type: "POST", 
				async:true,
				url: "<?php echo Yii::app()->baseUrl;?>/adminthanks/getusers/?unit_id="+unit_id+"&position_id="+position_id,    				
           ...
           setCookie("thanks_regist_comment",comment); 
           ...
           $('button[type="submit"]').click(function(){  
			...
			$.ajax({    
				type: "POST", 
				async:true,
				url: "<?php echo Yii::app()->baseUrl;?>/adminthanks/regist/",    
				...
				success: function(msg){	                        					  		
					  ...
					  jQuery('#Thanks_comment').prev().remove();
					  	...
					  	if(msg!='[]'){
                                                ...
                                                val=val.replace(/\n/g, "<br/>");
                                                setCookie("thanks_regist_comment",val);

...
<div class="wrap admin secondary ideas">
        ...
        $link_array[]=array("text"=>  Config::TEXT_FOR_DETAIL_IN_LINK_BACK_DIV,"link"=>'/adminideas/detail') ; 
        $this->widget('Path_actionwidget', array('home_link'=>$home_link,'link_array'=>$link_array)); ?> 
            <div class="mainBox detail">
                ...
                <?php 
					...
					?>
                <span><a href="<?php echo Yii::app()->request->baseUrl; ?>/adminideas/<?php echo $page?>" class="btn btn-important"><i class="icon-chevron-left icon-white"></i> <?php echo Config::TEXT_FOR_BACK_TO_LIST;?></a></span>        
                <span><a href="<?php echo Yii::app()->request->baseUrl; ?>/adminideas/edit/?id=<?php echo $model->id; ?>" class="btn btn-work"><i class="icon-pencil icon-white"></i><?php echo Config::TEXT_FOR_EDIT_IN_PAGE_DETAIL;?></a></span>
                </div>
                <div class="box">
                    ...
                    <div class="evaluate">
						...
						?>
                        <p class="rating"> Đánh giá：(<?php echo $average?>)</p>
					...
					<div class="photo">
                        <div class="imgbox">                            
                            	  <?php 
								   if(trim($attachment1)!="")
								   {	
										 FunctionCommon::echoOldFile($attachment1_ext, 1, $model,"adminideas",Yii::app()->request->baseUrl);
                                   }
                                   ?>
                        </div>
                        <div class="imgbox">
								   ...
								   if(trim($attachment2)!="")
								   {
								 		  FunctionCommon::echoOldFile($attachment2_ext, 2, $model,"adminideas",Yii::app()->request->baseUrl);	     
                                   }
                                   ?>
                        </div>
                        <div class="imgbox">
								   ...
								   if(trim($attachment3)!="")
								   {
								 		  FunctionCommon::echoOldFile($attachment3_ext, 3, $model,"adminideas",Yii::app()->request->baseUrl);	
                                   }
                                   ?>
                        </div>
                    </div>                   
					...
					<ul class="comments">
						...
						<li>
							...
							<div class="commenter row">
                            	...
                            	<div style="float:left;margin-left: 100px;">Đánh giá:(<?php echo $comment->valuation;?>)</div>
								...
								<div style="float:left;margin-left: 20px;">Thời gian gửi：<?php echo FunctionCommon::formatDate($comment->created_date); ?> <?php echo FunctionCommon::formatTime($model->created_date); ?></div>
                                                                <div style="clear: both;"></div>
							</div>
                                                        <div class="btn-comment-remove row"> 
									<div class="offset6 span1">
                                                                            <a class="btn btn-warning span2" onclick="if(confirm('<?php echo Config::TEXT_FOR_CONFIRM_DELETE;?>')==true) window.location='<?php echo Yii::app()->request->baseUrl; ?>/adminideas/deleteidideascomment/?id=<?php echo $model->id; ?>&id2=<?php echo $comment['id']; ?>';" style="width:111px; color:#ffffff; font-family:Helvetica Neue,Helvetica,Arial,sans-serif; text-decoration:none; cursor:pointer;">Xóa</a>
									</div>
							</div>
						</li>
						<?php		

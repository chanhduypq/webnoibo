...
class AdmincriticismController extends Controller {
    ...
    public function init() {
        parent::init();
        $this->pageTitle = "Các văn bản & quyết định";
    ...
    public function actionIndex() {
        ...
        $ide = Criticism::model()->findAll($criteria);
        $this->render('/admin/criticism/index', array(
            'ide' => $ide,
    ...
    public function actionEdit() {
        ...
        if ($model == null || !isset($_GET['id'])) {
            $this->redirect(array('admincriticism/index'));
        }
        ...
        $this->render('/admin/criticism/edit', $parmas);
    }
    /**
     * 
     */
    ...
    public function actionEditconfirm() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
                Upload_file_common_new::processAttachments($model, 'criticism', 2);
            }
            ...
            if ($model->validate()) {
                if (isset($_POST['edit']) && $_POST['edit'] == '1') {
                    ...
                    if ($model->save() == true) {
                        ...
                        $this->redirect(array('admincriticism/' . $page . ''));
                    }
                }
            } else {
                ...
                if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
                    if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1, 'criticism')) {
                ...
                if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
                    if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2, 'criticism')) {
                ...
                if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
                    if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3, 'criticism')) {
                ...
                $this->redirect(array('admincriticism/edit/?id=' . $model->id));
            }
        } else {
            $this->redirect(array('admincriticism/index'));
        }
        $this->render('/admin/criticism/editconfirm', array('model' => $model));
    }
    ...
    public function actionDetail() {
        ...
        if (!empty($model->title)) {
            $this->render('/admin/criticism/detail', array(
                'model' => $model
                    )
            );
        } else {
    ...
    public function loadModel() {
        if ($this->_criticism === null) {
            ...
            } else if (isset($_POST['Criticism'])) {
                $data = $_POST['Criticism'];
                $id = $data['id'];
                $this->_criticism = Criticism::model()->findbyPk(intval($id));
            } else {
    ...
    public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('criticism_edit_from') ? Yii::app()->request->cookies['criticism_edit_from']->value : '';

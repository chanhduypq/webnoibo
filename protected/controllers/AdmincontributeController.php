...
class AdmincontributeController extends Controller {
    ...
    public function actionIndex() {
        ...
        $ide = Contribute::model()->findAll($criteria);
        $this->render('/admin/contribute/index', array(
            'ide' => $ide,
    ...
    public function actionEdit() {
        ...
        if ($model == null || !isset($_GET['id'])) {
            $this->redirect(array('admincontribute/index'));
        }
        ...
        $this->render('/admin/contribute/edit', $parmas);
    }
    /**
     * 
     */
    ...
    public function actionEditconfirm() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
                Upload_file_common_new::processAttachments($model, 'contribute', 2);
            }
            ...
            if ($model->validate()) {
                if (isset($_POST['edit']) && $_POST['edit'] == '1') {
                    ...
                    if ($model->save() == true) {
                        ...
                        $this->redirect(array('admincontribute/' . $page . ''));
                    }
                }
            } else {
                ...
                if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
                    if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1, 'contribute')) {
                ...
                if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
                    if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2, 'contribute')) {
                ...
                if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
                    if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3, 'contribute')) {
                ...
                $this->redirect(array('admincontribute/edit/?id=' . $model->id));
            }
        } else {
            $this->redirect(array('admincontribute/index'));
        }
        $this->render('/admin/contribute/editconfirm', array('model' => $model));
    }

    

    /**
     * Detail id contribute
     */
    public function actionDetail() {
        ...
        if (!empty($model->title)) {
            $this->render('/admin/contribute/detail', array(
                'model' => $model
                    )
            );
        } else {
    ...
    public function loadModel() {
        if ($this->_contribute === null) {
            ...
            } else if (isset($_POST['Contribute'])) {
                $data = $_POST['Contribute'];
                $id = $data['id'];
                $this->_contribute = Contribute::model()->findbyPk(intval($id));
            } else {
    ...
    public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('contribute_edit_from') ? Yii::app()->request->cookies['contribute_edit_from']->value : '';

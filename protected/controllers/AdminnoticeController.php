...
class AdminnoticeController extends Controller {
    ...
    public function actionIndex() {
        ...
        $ide = Notice::model()->findAll($criteria);
        $this->render('/admin/notice/index', array(
            'ide' => $ide,
    ...
    public function actionEdit() {
        ...
        if ($model == null || !isset($_GET['id'])) {
            $this->redirect(array('adminnotice/index'));
        }
        $count=Yii::app()->db->createCommand()
                    ...
                    ->queryScalar();
            if($count=='0'){
                Yii::app()->db->createCommand("insert into notice_read values (".$model->id.",".Yii::app()->request->cookies['id'].")")->execute();
            }
        ...
        $this->render('/admin/notice/edit', $parmas);
    }
    ...
    public function actionEditconfirm() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
                Upload_file_common_new::processAttachments($model, 'notice', 2);
            }
            ...
            if ($model->validate()) {
                if (isset($_POST['edit']) && $_POST['edit'] == '1') {
                    ...
                    if ($model->save() == true) {
                        ...
                        $this->redirect(array('adminnotice/' . $page . ''));
                    }
                }
            } else {
                ...
                if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
                    if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1, 'notice')) {
                ...
                if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
                    if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2, 'notice')) {
                ...
                if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
                    if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3, 'notice')) {
                ...
                $this->redirect(array('adminnotice/edit/?id=' . $model->id));
            }
        } else {
            $this->redirect(array('adminnotice/index'));
        }
        $this->render('/admin/notice/editconfirm', array('model' => $model));
    }
    ...
    public function actionDetail() {
        ...
        if (!empty($model->title)) {
            ...
            $this->render('/admin/notice/detail', array(
                'model' => $model
                    )
            );
        } else {
    ...
    public function loadModel() {
        if ($this->_notice === null) {
            if (isset($_GET['id'])) {
                ...
                $data = $_POST['Notice'];
                $id = $data['id'];
                $this->_notice = Notice::model()->findbyPk(intval($id));
            } else {
    ...
    public function actionDelete() {
        ...
        ;
        Yii::app()->db->createCommand("delete from notice_read where notice_id=$id")->execute();
    ...
    public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('notice_edit_from') ? Yii::app()->request->cookies['notice_edit_from']->value : '';

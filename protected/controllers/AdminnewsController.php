...
class AdminnewsController extends Controller {
    ...
    public function actionIndex() {
        ...
        $ide = News::model()->findAll($criteria);
    ...
    public function actionEdit() {
        ...
        $count=Yii::app()->db->createCommand()
                    ...
                    ->queryScalar();
            if($count=='0'){
                Yii::app()->db->createCommand("insert into news_read values (".$model->id.",".Yii::app()->request->cookies['id'].")")->execute();
            }
        ...
        $this->render('/admin/news/edit', $parmas);
    }
    ...
    public function actionEditconfirm() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
                Upload_file_common_new::processAttachments($model, 'news', 2);
            }
            ...
            if ($model->validate()) {
                if (isset($_POST['edit']) && $_POST['edit'] == '1') {
                    ...
                    if ($model->save() == true) {
                        ...
                        $this->redirect(array('adminnews/' . $page . ''));
                    }
                }
            } else {
                ...
                if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
                    if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1, 'news')) {
                ...
                if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
                    if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2, 'news')) {
                ...
                if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
                    if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3, 'news')) {
                ...
                $this->redirect(array('adminnews/edit/?id=' . $model->id));
            }
        } else {
            $this->redirect(array('adminnews/index'));
        }
        $this->render('/admin/news/editconfirm', array('model' => $model));
    }
    ...
    public function actionDetail() {
        ...
        if (!empty($model->title)) {
            ...
            $this->render('/admin/news/detail', array(
                'model' => $model
                    )
            );
        } else {
    ...
    public function loadModel() {
        if ($this->_news === null) {
            if (isset($_GET['id'])) {
                ...
                $data = $_POST['News'];
                $id = $data['id'];
                $this->_news = News::model()->findbyPk(intval($id));
            } else {
                $this->_news = new News();
            }
        }
    ...
    public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('news_edit_from') ? Yii::app()->request->cookies['news_edit_from']->value : '';

...
class AdminitcontestController extends Controller {
    ...
    public function actionIndex() {
        ...
        $ide = Itcontest::model()->findAll($criteria);
        $this->render('/admin/itcontest/index', array(
            'ide' => $ide,
    ...
    public function actionEdit() {
        ...
        if ($model == null || !isset($_GET['id'])) {
            $this->redirect(array('adminitcontest/index'));
        }
        ...
        $this->render('/admin/itcontest/edit', $parmas);
    }
    /**
     * 
     */
    ...
    public function actionEditconfirm() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_POST['edit']) || $_POST['edit'] != '1') {
                Upload_file_common_new::processAttachments($model, 'itcontest', 2);
            }
            ...
            if ($model->validate()) {
                if (isset($_POST['edit']) && $_POST['edit'] == '1') {
                    ...
                    if ($model->save() == true) {
                        ...
                        $this->redirect(array('adminitcontest/' . $page . ''));
                    }
                }
            } else {
                ...
                if ($model->attachment1 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment1)) {
                    if ($model->attachment1 != Upload_file_common::getAttachmentById($model->id, 1, 'itcontest')) {
                ...
                if ($model->attachment2 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment2)) {
                    if ($model->attachment2 != Upload_file_common::getAttachmentById($model->id, 2, 'itcontest')) {
                ...
                if ($model->attachment3 != "" && file_exists(Yii::getPathOfAlias('webroot') . $model->attachment3)) {
                    if ($model->attachment3 != Upload_file_common::getAttachmentById($model->id, 3, 'itcontest')) {
                ...
                $this->redirect(array('adminitcontest/edit/?id=' . $model->id));
            }
        } else {
            $this->redirect(array('adminitcontest/index'));
        }
        $this->render('/admin/itcontest/editconfirm', array('model' => $model));
    }
    ...
    public function actionDetail() {
        ...
        if (!empty($model->title)) {
            $this->render('/admin/itcontest/detail', array(
                'model' => $model
                    )
            );
        } else {
    ...
    public function loadModel() {
        if ($this->_itcontest === null) {
            ...
            } else if (isset($_POST['Itcontest'])) {
                $data = $_POST['Itcontest'];
                $id = $data['id'];
                $this->_itcontest = Itcontest::model()->findbyPk(intval($id));
            } else {
    ...
    public function filters() {
        $backCookie = Yii::app()->request->cookies->contains('itcontest_edit_from') ? Yii::app()->request->cookies['itcontest_edit_from']->value : '';
